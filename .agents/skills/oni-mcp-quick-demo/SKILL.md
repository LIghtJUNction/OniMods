...
```
navigation_control action=jump code=home displayText="ONI MCP 演示开始：先展示只读、预览和 dry-run 能力。"
```
...
```
read_control domain=knowledge kind=database action=query query="Electrolyzer" includeContent=true limit=5
```
...
```
read_control domain=knowledge kind=database action=query query="oxygen generation oxygen production" includeContent=true limit=5
```
...
```
navigation_control action=jump code=mouse
navigation_control action=select_tool tool=dig
navigation_control action=left_click dryRun=true displayText="演示：预检挖掘此格"
navigation_control action=select_tool tool=build prefabId=Tile material=auto
navigation_control action=left_click dryRun=true displayText="演示：预检在此铺砖"
```
