    ...
    private static void RunHeaderlessModernCancellationRegression()
    {
        ...
        try
        {
            ...
            })
            {
                ...
                {
                    Assert(response.StatusCode == HttpStatusCode.BadRequest,
                        "Headerless modern cancellation was accepted instead of rejected");
                    Assert(ReadJson(response)["error"] != null,
                        "Headerless modern cancellation did not return a JSON-RPC error");
                    Assert(!response.Headers.Contains("Mcp-Session-Id"),
                        "Rejected headerless modern cancellation returned a legacy session id");
                }

                Assert(server.GetSessionSummaries().Count == 0,
                    "Rejected headerless modern cancellation allocated legacy session state");
            }
        }
        finally
        {
