...
internal static class ModernMrtrResourceSizeRegressionEntry
{
    private static MainThreadBridge _bridge;

    private static void Main()
    {
        RunModernMrtrResourceSizeRegression();

        var existing = typeof(ModernEmptyExtensionIdentifierRegressionEntry).GetMethod("Main",
    ...
    private static void RunModernMrtrResourceSizeRegression()
    {
        ...
        try
        {
            ...
            })
            {
                ...
                AssertModernRejected(client,
                    BuildBenchmarkCall(53101, "\"large\""),
                    "resource_link with non-numeric size");
                Assert(OniToolRegistry.Calls == callsBefore,
                    "Non-numeric resource_link size reached tool dispatch");

                AssertModernAccepted(client,
                    BuildBenchmarkCall(53102, "1.5"),
                    "resource_link with schema-valid fractional size");
                Assert(OniToolRegistry.Calls == callsBefore + 1,
                    "Schema-valid fractional resource_link size did not dispatch exactly once");
                Assert(server.GetSessionSummaries().Count == 0,
                    "Modern resource_link size validation allocated legacy session state");
            }
        }
        finally
        {
    ...
    private static string BuildBenchmarkCall(int id, string sizeJson)
    {
        return "{\"jsonrpc\":\"2.0\",\"method\":\"tools/call\",\"id\":" + id
            + ",\"params\":{\"inputResponses\":{\"probe\":{\"role\":\"assistant\",\"content\":{"
            + "\"type\":\"tool_result\",\"toolUseId\":\"call-size\",\"content\":[{"
            + "\"type\":\"resource_link\",\"name\":\"fixture\",\"uri\":\"https://example.test/resource\","
            + "\"size\":" + sizeJson + "}]},\"model\":\"fixture\"}},"
            + "\"name\":\"benchmark\",\"arguments\":{\"task\":\"resource size regression\",\"iterations\":1},"
            + ModernMeta() + "}}";
    }
    ...
    private static string ModernMeta()
    {
        return "\"_meta\":{\"io.modelcontextprotocol/protocolVersion\":\"2026-07-28\","
            ...
            + "\"io.modelcontextprotocol/clientInfo\":{\"name\":\"resource-size-regression\",\"version\":\"1.0\"}}";
    }
