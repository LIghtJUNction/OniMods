...
Reports are cumulative histograms within one game-session capture generation. A new `Game.Instance` resets the fixed histogram and sparse-report thresholds, and each emitted summary carries `captureGeneration`. Formatting/logging is deferred through `UIScheduler.ScheduleNextFrame`; the `UpdateGraph` prefix only scans the existing dirty list, performs the cheap game-boundary check, and updates fixed-size counters. `UIScheduler` uses ONI's unscaled UI clock, so a report that was requested while the simulation was running can still flush after the game is paused. Reports are requested after call 1, 64, 256, 1024, and then every 4x growth. The normal report shows only the 12 most common dirty-count/range/density buckets so ordinary developer logs stay readable.

The deferred callback is tied to both the capture generation and the `UIScheduler` instance that scheduled it. Save/load therefore cannot leave the process-static `reportScheduled` flag stranded on a destroyed scheduler, and a stale callback from an older game generation cannot publish or clear a newer generation's report.
...
`--after-calls` must match an existing complete capture in the latest `captureGeneration`. The analyzer subtracts the baseline complete histogram from the later complete histogram and returns only the fresh workload window: top-level `calls`, `empty`, `buckets`, and `candidateGate` describe the delta, while `baselineCalls` and `cumulativeCalls` preserve the two cumulative endpoints. Bucket counts are required to stay monotonic and their deltas must sum exactly to the fresh call count. A save/load generation change, a legacy capture without generation metadata, a missing baseline, or a window that never emitted a fresh deferred report fails closed instead of reusing old workload evidence.

The JSON also reports `candidateGate` bounds for the current synthetic `NavGridAdaptiveGateBenchmark` rule (`dirty >= 20`, short range `>= 2`, long range `>= 4`). The runtime histogram deliberately keeps coarse buckets such as `dirty=16-23`, so a capture cannot always say exactly how many calls would satisfy that candidate rule. `eligibleCallsLowerBound` counts only buckets that are guaranteed to satisfy it, `eligibleCallsUpperBound` includes threshold-crossing buckets that might satisfy it, and `ambiguousCalls` is the difference. In `--after-calls` mode these bounds are computed from the fresh histogram delta, not the process/session history. These are workload-coverage bounds, not a production routing decision or performance claim. The contract check keeps the analyzer thresholds synchronized with the benchmark constants.
