    ...
    public static partial class GameControlTools
    {
        public static McpTool ControlGame()
        {
            return new McpTool
            {
                ...
                Tags = new List<string> { "game", "speed", "pause", "save", "launch", "dlc", "red-alert", "sandbox", "debug", "map", "ui", "feedback", "edit-mark" },
                Description = "统一游戏入口。domain=launch/speed/state/save/dlc/sandbox/ui；launch 可自动加载最近或指定存档开始可控游戏会话；sandbox 下 kind=read/area/entity/map_designate 用于沙盒读取、区域编辑、实体生成和 search/designate 地图编辑；ui 下 uiDomain=action/feedback/edit_mark。",
                Parameters = new Dictionary<string, McpToolParameter>
                {
                    ...
                    ["path"] = new McpToolParameter { Type = "string", Description = "domain=save action=load 或 domain=launch action=start 时完整存档路径", Required = false },
                    ...
                    ["resume"] = new McpToolParameter { Type = "boolean", Description = "domain=launch action=start：加载后/已在游戏内时是否解除暂停，默认 true", Required = false },
