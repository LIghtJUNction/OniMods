        ...
        public static McpTool ControlBuilding()
        {
            return new McpTool
            {
                ...
                Parameters = new Dictionary<string, McpToolParameter>
                {
                    ...
                    ["action"] = new McpToolParameter { Type = "string", Description = "子动作。planning=search_defs/materials/preview/placement_candidates/auto_connect/build_area；config=list/list_automation/set_*；production=list_fabricators/list_recipes/set/batch/mutant_seed_*；storage=list/detail/set_filter；filter=list/set；tile_selection=list/set/batch；receptacle=list/request/cancel_request/remove_occupant/cancel_remove/batch；side_surface=list/press/focus/batch；rocket=ops/module/flight_utility/restriction/usage/crew_request/assignment_group/cargo_status/self_destruct 的子动作；facility=list/set/assign/consume 等", Required = false },
                    ...
                    ["prefabId"] = new McpToolParameter { Type = "string", Description = "planning 材料/预览/候选目标建筑 prefabId", Required = false },
