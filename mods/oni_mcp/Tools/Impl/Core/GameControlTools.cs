    ...
    public static partial class GameControlTools
    {
        public static McpTool ControlGame()
        {
            return new McpTool
            {
                ...
                Description = "统一游戏入口。domain=launch/speed/state/save/dlc/sandbox/ui；launch 可自动加载最近或指定存档开始可控游戏会话；sandbox 下 kind=read/area/entity/map_designate 用于沙盒读取、区域编辑、实体生成和 search/designate 地图编辑；ui 下 uiDomain=action/feedback。",
