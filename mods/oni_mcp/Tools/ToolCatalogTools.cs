        ...
        private static List<ToolGuide> BuiltInGuides()
        {
            return new List<ToolGuide>
            {
                new ToolGuide("general",
                    ...
                    new[] { "tools_search", "tools_manifest", "tools_call_many", "plan_harness_create" },
                    new[] { "tools_search detail=brief query=<goal>", "tools_guide goal=<goal>" },
                    "Use tools_call_many for independent read checks; keep write/execute tools explicit.",
                    new[] { "read colony summary", "search tools", "create plan", "execute", "verify" }),
                new ToolGuide("map_area_orders",
                    new[] { "dig", "sweep", "mop", "disinfect", "cancel", "attack", "harvest", "地图", "挖掘", "清扫", "区域" },
                    new[] { "oni://world/text-map{?profile=scan,format=json}", "oni://tools/read/priorities_list" },
                    new[] { "world_text_map", "orders_dig_area", "orders_sweep_area", "orders_mop_area", "orders_cancel_area", "orders_harvest_area", "tools_call_many" },
                    new[] { "orders area batch", "dig sweep mop harvest cancel" },
                    "Batch same-area reads and independent area commands; each dangerous command still needs confirm.",
                    new[] { "read world_text_map profile=scan format=json", "define area", "plan orders", "execute confirmed area tool", "verify with text map" }),
                ...
                new ToolGuide("build_and_configure",
                    ...
                    new[] { "buildings_search_defs", "buildings_materials", "buildings_plan", "buildings_plan_rect", "buildings_plan_many", "buildings_config_list", "buildings_config_batch_set", "tools_call_many" },
                    new[] { "buildings_search_defs query=toilet", "buildings_materials prefabId=Outhouse", "buildings_plan prefabId=Outhouse material=auto", "building automation threshold" },
                    "Use buildings_search_defs to choose prefab/facade. Prefer material=auto; call buildings_materials before explicit material. Invalid or unavailable materials return structured candidates for retry. Use buildings_plan_many for multiple blueprints, then config batch tools after placement.",
                    new[] { "scan area", "search defs", "use material=auto or read materials", "plan blueprint", "verify placement", "batch config if needed" }),
                ...
                new ToolGuide("resources_food_storage",
                    ...
                    new[] { "resources_inventory", "resources_food", "resources_pins_list", "resources_pin_set", "colony_diagnostic_settings_list", "colony_diagnostic_settings_set", "resources_storage_list", "resources_storage_set_filter", "filters_list", "filters_tree_set", "diet_status", "diet_policy" },
                    new[] { "resources food storage filter diet pin notify diagnostics alerts" },
                    "Batch read inventory/storage/filter/diagnostic state; apply storage/filter, AllResourcesScreen pin/notify, and AllDiagnosticsScreen settings with explicit target ids.",
                    new[] { "read inventory and storage", "identify shortage", "plan filter/diet/storage changes", "execute", "verify" }),
