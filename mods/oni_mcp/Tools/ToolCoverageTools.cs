    ...
    public static class ToolCoverageTools
    {
        ...
        private static List<CoverageRow> BuildRows()
        {
            return new List<CoverageRow>
            {
                ...
                Row("planning", "plan_feedback_verify_execute_harness", "规划、反馈、验证、约束记录、门禁校验和通过 plannedCalls 执行实施", "covered", "plan_harness_create", "plan_harness_record", "plan_harness_validate", "plan_harness_execute", "plan_harness_get", "plan_harness_list"),
                ...
                Row("buildings", "build_plan_place", "搜索可建造物、材料和建筑外观，并放置单个/矩形/批量蓝图", "covered", "buildings_search_defs", "buildings_plan", "buildings_plan_rect", "buildings_plan_many"),
