...
> **API 稳定性警告**：`oni_mcp` 在 `1.0.0` 之前仍处于快速迭代阶段，工具名称、参数、资源路径和返回字段都可能发生不兼容变更。二创、插件、脚本或第三方客户端请锁定具体版本，并以运行时 `server_control domain=catalog action=manifest` / `oni://tools/manifest` 为准做兼容适配。
...
当前实现包含 330+ 个工具、120+ 个固定资源和 100+ 个资源模板。工具和资源会随代码继续变化，实际清单以运行时 `server_control domain=catalog action=manifest` 和 `oni://tools/manifest` 为准。

### 核心工具

| 范围 | 代表工具 | 用途 |
|------|----------|------|
| 服务与目录 | `server_control`, `server_control domain=catalog action=manifest`, `server_control domain=catalog action=search`, `server_control domain=catalog action=guide` | 检查服务、搜索工具、按目标生成工具指南 |
| 机制知识 | `read_control domain=knowledge kind=guide action=query`, `read_control domain=knowledge kind=database action=query` | 查询玩家机制/公式速查，并对照游戏内置百科 |
| 游戏控制 | `game_control domain=speed`, `game_control domain=save` | 暂停、恢复、调速、存档 |
| 世界与区域读取 | `read_control domain=world action=text_map`, `read_control domain=world action=area_snapshot`, `read_control domain=world action=cell_info`, `read_control domain=area` | 文本地图、区域快照、格子详情；定义、读取、分块、拼接和复用地图区域 |
| 导航、截图与指针 | `navigation_control` | 移动视角、切换覆盖层、截图；用可视 agent 指针定位、选工具、点击/拖拽和读取玩家鼠标格 |
| 建筑与订单 | `building_control domain=planning`, `orders_control` | 搜建筑、找可放锚点、选材料、预检蓝图、自动连接管线/电线、下达区域订单 |
| 复制人 | `dupes_control domain=info`, `dupes_control domain=priority`, `dupes_control domain=command action=rename`, `dupes_control domain=command action=auto_rename` | 状态检查、详情、属性、需求、优先级、单人改名和按职业自动命名 |
| 管理界面 | `colony_control domain=management`, `building_control domain=storage` | 日程、饮食、研究、医疗、储存过滤和管理屏设置 |
| 设施侧屏 | `building_control domain=config`, `building_control domain=filter`, `building_control domain=config action=state_list`, `building_control domain=side_surface surface=option`, `building_control domain=config action=visual kind=light` | 常见建筑侧屏配置 |
| 火箭与太空 | `building_control domain=rocket rocketDomain=ops`, `building_control domain=rocket rocketDomain=module`, `building_control domain=rocket rocketDomain=restriction`, `building_control domain=rocket rocketDomain=crew_request` | 火箭状态、模块、限制、乘员和太空设施 |
| 审计与覆盖 | `server_control domain=catalog action=coverage`, `server_control domain=catalog action=static_audit`, `server_control domain=catalog action=surface_audit surface=side_screen` | 检查工具覆盖面和缺口 |
| 批量与规划 | `server_control domain=batch action=call_many`, `server_control domain=program action=execute`, `game_control domain=ui uiDomain=edit_mark` | 批量调用、条件/循环流程脚本、读取玩家标记 |

`navigation_control` 使用全局逻辑指针名；不传 `agentId` 时使用默认 `agent` 指针。相同 `agentId` 跨 MCP session 复用同一个指针，避免客户端重连后留下重复指针；建议多步操作第一步用 `action=get` 创建/读取一个短名指针（如 `planner`、`builder`），之后持续传同一个 `agentId`。可见指针动作尽量传 `displayText`，在指针旁用短句告诉玩家正在定位、选工具或执行什么。需要多个并行指针时传不同 `agentId`。不再需要某个指针时，用 `navigation_control action=clear` 删除它及其跳转点。回到主菜单或游戏世界未加载时，指针会自动隐藏。
...
2. 再找工具: 用 `server_control domain=catalog action=search` 或 `oni://tools/guide` 根据目标检索工具。
3. 小范围操作: 对区域先 `read_control domain=area action=define`，再使用支持 `areaId` 的工具，避免坐标误伤。
...
5. 用 `server_control domain=catalog action=static_audit` 或 `server_control domain=catalog action=manifest` 检查运行时注册结果。
